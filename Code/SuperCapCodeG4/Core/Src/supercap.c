...
void TIM2_NVIC(){
  ...
  */
  Supercap_First_Order_Filter_ADC_Function_Alpha(&C_left, 2);
  Supercap_First_Order_Filter_ADC_Function_Alpha(&C_sys, 2);
  Supercap_First_Order_Filter_ADC_Function_Alpha(&C_right, 2);
...
void Supercap_Average_ADC_Function(_ADC_Sample_t *adc_sample) {
    ...
    adc_sample->real_value_12bits = ADC_to_Voltage_Funtion(adc_sample->average);;
}
